...
# class MultiHeadAttention(nn.Module):
#     def __init__(self, hidden_dimension,embedding_dim, n_head,dropout_rate,mask=False):
#         super().__init__()
        
#         d_k = int(hidden_dimension/n_head)
#         self.Q_ = nn.Linear(embedding_dim, d_k)
#         self.K_ = nn.Linear(embedding_dim, d_k)
#         self.V_ = nn.Linear(embedding_dim, d_k)
        
#         self.scaled_dot_product_attenion = nn.ModuleList([ScaledDotProductAttention(hidden_dimension, n_head) for _ in range(n_head) ])
#         self.out_linear = nn.Linear(hidden_dimension,hidden_dimension)
...
#         init.xavier_uniform_(self.out_linear.weight)
#         init.zeros_(self.out_linear.bias)
#     def forward(self, q_, k_, v_, attn_mask=None, dec_mask = None):
...
#         attention = [head(q,k,v,attn_mask,dec_mask) for head in self.scaled_dot_product_attenion]
        
#         concat_attention = torch.cat(attention, dim =2) # [256, 9, 64]
