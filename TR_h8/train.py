...
def training(model, train_dataloader, criterion, optimizer, scheduler, start_step, train_losses, train_ppls,tokenizer):
    step = start_step
    ...
    with tqdm(total=int(config.total_steps), desc="Training") as pbar:
        while step <= config.total_steps:
            ...
            for train_batch in train_dataloader:
                ...
                # Save Checkpoint
                if (step % 10000 == 0 or (step == config.total_steps) or ((step) in [98500, 97000, 95500, 94000])) and step != 0:
...
def main():
    ...
    training(model, train_data_loader, criterion, optimizer, scheduler,start_step,train_losses, train_ppls,tokenizer)
